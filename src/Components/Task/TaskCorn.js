...
const TaskCorn = ({ showModal, setShowModal }) => {
  ...
  const taskID = "task_corn"; // Assign a unique ID to this task
  ...
  const handleTaskLinkClick = () => {
    window.open("https://t.me/Corn/PlayCornBattles?startapp=6624399776");
  ...
  const handleVerify = async () => {
    ...
    const response = await fetch(
      `https://api.telegram.org/bot${process.env.REACT_APP_TELEGRAM_BOT_TOKEN}/getChatMember?chat_id=-1001379581154&user_id=${id}`
    );
        ...
        <div className="fixed z-50 left-0 right-0 top-0 bottom-0 flex justify-center taskbg px-[16px] h-full">
          <div className={`w-full flex flex-col items-center justify-start`}>
            ...
            <div className="flex flex-col w-full">
              <h1 className="text-[20px] font-semibold">Play CORN</h1>
              <p className="text-[#9a96a6] text-[16px] font-medium pt-1 pb-10">
                Play the $CORN and earn future token!
              </p>
